...
/*** TO BE DONE START ***/
		 						    //per i test su getaddrinfo
	nr=getaddrinfo(argv[1],argv[2],&gai_hints,&server_addrinfo); //decodifica il nome del sito passato su argv[1] e la port su argv[2],
								    //attraverso i suggerimenti contenuti nella struct gai_hints, e scrive nella 
								    //struct *addrinfo server_addrinfo
	if (nr!=0){
		printf("Failed getting address info\n");
		exit(EXIT_FAILURE);
	}
...
/*** TO BE DONE START ***/
	tcp_socket = socket(AF_INET,SOCK_STREAM,0);		     //apro socket con protocollo AF_INET (ipv4), di tipo STREAM e scrivo il file 
								     //descriptor su tcp_socket
	if (tcp_socket<0){
		printf("Failed creating socket\n");
		exit(EXIT_FAILURE);
	}

	nr=connect(tcp_socket,(struct sockaddr*) ipv4, sizeof(struct sockaddr_in));
								     //connetto attraverso il socket appena aperto, con le istruzioni ottenute da 
								     //server_addrinfo, facendo un cast a sockaddr* in quanto richiesto dalla 
								     //connect, passando la size di ipv4
	if (nr==-1){
		printf("Failed in connection establishment: %s\n",strerror(errno));
		exit(EXIT_FAILURE);
	}
...
/*** TO BE DONE START ***/
	...
	nr=send(tcp_socket,request,sizeof(request),0);                            //invio attraverso tcp_socket il buffer request di dimensione 
	if (nr==-1){                                                               //sizeof(request)
		printf("Failed sending request: %s\n",strerror(errno));
		exit(EXIT_FAILURE);
	}
